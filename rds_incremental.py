...
file = 0
for i in object.list_objects(Bucket='group-1-bucket')['Contents']:
    file+=1
    print(i['Key'])
print(file)
...
if file>1:
    ...
    df1.coalesce(1).write.mode("append").option("header","true").format("csv").save("s3a://group-1-bucket/Clean_Zone/structure_RDS/")
else:
